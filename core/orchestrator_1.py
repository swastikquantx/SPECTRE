...
class SpectreOrchestrator:
    ...
    async def understand(self, objective: str) -> Dict:
        ...
        try:
            return json.loads(out["output"]) if isinstance(out["output"], str) else out["output"]
        except:
            return out.get("output", {"intent": objective})
    ...
    async def research(self, understanding: Dict) -> Dict:
        ...
        adapter = await self.registry.get_adapter(ranked[0].id)
        out = await adapter.execute({"type": "research", "query": understanding["intent"]}, self.state)
    ...
    async def plan(self, understanding: Dict, research: Dict) -> Dict:
        ...
        try:
            ...
            return {"tasks": [{"id": "t1", "description": understanding["intent"], "capability": "coding"}]}
